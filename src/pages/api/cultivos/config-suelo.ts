  ...
  try {
    ...
    if (method === "PUT") {
      ...
      return new Response(
        JSON.stringify({ 
          success: true, 
          message: "Configuración de suelo actualizada correctamente",
          data: suelo
        }),
    ...
    // GET para obtener la configuración de suelo de un cultivo específico
    if (method === "GET") {
      ...
      return new Response(
        JSON.stringify({ success: true, data: data.suelo }),
        { status: 200, headers: { "Content-Type": "application/json" } }
      );
    }
    ...
    console.error("Error en ALL /api/cultivos/config-suelo:", err);
    return new Response(
